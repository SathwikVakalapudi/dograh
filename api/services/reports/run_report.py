...
def _sum_llm_tokens(usage: dict[str, Any]) -> int:
    """Total LLM tokens (prompt + completion) across all llm entries in usage_info."""
    total = 0
    for entry in (usage.get("llm") or {}).values():
        if isinstance(entry, dict):
            total += (entry.get("prompt_tokens") or 0) + (entry.get("completion_tokens") or 0)
    return int(total)


def _sum_tts_characters(usage: dict[str, Any]) -> int:
    """Total TTS characters across all tts entries in usage_info."""
    total = 0
    for value in (usage.get("tts") or {}).values():
...
def build_run_report_csv(runs: List[Any]) -> io.StringIO:
    ...
    for run in runs:
        ...
        pre_values = [
            ...
            _sum_llm_tokens(usage),
            _sum_tts_characters(usage),
        ]
