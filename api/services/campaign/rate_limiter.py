...
class RateLimiter:
    ...
    async def store_channel_run_mapping(
        self, channel_id: str, workflow_run_id: int
    ) -> bool:
        """Store an ARI channel_id -> workflow_run_id mapping at origination.

        ARI has no status webhook, so a campaign call that never enters Stasis
        (no-answer/busy/failed) is only observed via ChannelDestroyed in
        ari_manager. Recording the mapping here lets that teardown release the
        caller-ID + concurrency slot immediately instead of waiting for the
        20-minute stale reclaim. Uses the same key convention as
        ari_manager._set_channel_run ("ari:channel:{channel_id}").
        """
        if not channel_id:
            return False
        try:
            ...
            await redis_client.set(
                f"ari:channel:{channel_id}", str(workflow_run_id), ex=3600
            )
            return True
        except Exception as e:
            logger.error(f"Error storing channel->run mapping: {e}")
            return False
