...
@node_spec(
    ...
    field_overrides={
        ...
        "greeting_type": {
            ...
            "description": (
                "Whether this step's opening line is spoken via TTS from text or "
                "played from a pre-recorded audio file."
            ),
            "options": [
                PropertyOption(value="text", label="Text (TTS)"),
        ...
        "greeting": {
            "display_name": "Opening Text",
            "description": (
                "Text spoken via TTS when this step begins. Supports "
                "{{template_variables}}. Leave empty to let the LLM open the step."
            ),
            "display_options": DisplayOptions(show={"greeting_type": ["text"]}),
            "placeholder": "And which party did you vote for in 2022?",
            "editor": "textarea",
        },
        "greeting_recording_id": {
            "display_name": "Opening Recording",
            "description": "Pre-recorded audio file played when this step begins.",
            "ui_type": PropertyType.recording_ref,
            "llm_hint": (
                "Value is the `recording_id` string. Use the `list_recordings` "
                "MCP tool to discover available recordings."
...
class AgentNodeData(
    ...
    # Mirrors StartCallNodeData. Without these an agent node cannot carry a
    # recording reference, so every mid-call question falls through to the LLM.
    greeting: Optional[str] = spec_field(default=None, ui_type=PropertyType.string)
    greeting_type: Optional[str] = spec_field(
        default=None, ui_type=PropertyType.options
    )
