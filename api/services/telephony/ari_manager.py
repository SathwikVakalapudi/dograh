...
# Marks a channel as having entered Stasis, i.e. answered. ChannelDestroyed
# uses it to tell a genuinely un-answered call from a completed one whose
# teardown task has not run yet.
_ANSWERED_KEY_PREFIX = "ari:answered:"
...
class ARIConnection:
    ...
    async def _mark_channel_answered(self, channel_id: str):
        """Record that a channel entered Stasis (was answered)."""
        r = await self._get_redis()
        await r.set(
            f"{_ANSWERED_KEY_PREFIX}{channel_id}", "1", ex=_CHANNEL_KEY_TTL
        )

    async def _was_channel_answered(self, channel_id: str) -> bool:
        """Whether this channel ever entered Stasis."""
        r = await self._get_redis()
        return await r.get(f"{_ANSWERED_KEY_PREFIX}{channel_id}") is not None
    ...
    async def _delete_channel_run(self, *channel_ids: str):
        ...
        keys += [f"{_ANSWERED_KEY_PREFIX}{cid}" for cid in channel_ids]
        await r.delete(*keys)
    ...
    async def _handle_event(self, raw_data: str):
        ...
        if event_type == "StasisStart":
            # Reaching Stasis means the channel was answered. Recorded before
            # any branch below returns, so the ext leg is covered too.
            await self._mark_channel_answered(channel_id)
            ...
            # An outbound call that never entered Stasis (no-answer/busy/failed)
            # is only observed here — ARI has no status webhook. If the
            # channel->run mapping still exists, StasisEnd never ran (answered
            # calls delete it during teardown, and events are processed
            # sequentially), so this is a terminal, un-answered call. Route it
            # through the shared status processor — same path webhook providers
            # use — to finalize the run, release the caller-ID/concurrency slot,
            # record the circuit breaker, and schedule a busy/no-answer retry.
            run_id = await self._get_channel_run(channel_id)
            if run_id and await self._was_channel_answered(channel_id):
                # Answered: the pipeline owns the disposition and teardown runs
                # on its own task. Finalizing here would overwrite a completed
                # run with a failure and schedule a pointless retry.
                logger.debug(
                    f"[ARI org={self.organization_id}] ChannelDestroyed for answered "
                    f"channel {channel_id} (run {run_id}); leaving disposition to the pipeline"
                )
                run_id = None
            ...
            if run_id:
                try:
                    ...
                    mapped_status = self._map_hangup_cause_to_status(cause, tech_cause)
                    await _process_status_update(
                        int(run_id),
                        StatusCallbackRequest(
                            call_id=channel_id, status=mapped_status
                        ),
                    )
                    logger.info(
                        f"[ARI org={self.organization_id}] Finalized un-answered run "
                        f"{run_id} as {mapped_status.value} (channel {channel_id})"
                    )
                except Exception as exc:
                    logger.warning(
                        f"[ARI org={self.organization_id}] ChannelDestroyed finalize "
    ...
    async def _handle_stasis_end(self, channel_id: str, workflow_run_id: str):
        ...
        try:
            ...
            # Release the campaign caller-ID (from_number) back to its pool.
            # ARI has no status webhook, so this StasisEnd teardown is where an
            # answered campaign call frees its caller ID — otherwise it stays
            # "in use" until the 20-minute stale reclaim, blocking the pool.
            try:
                from api.services.campaign.rate_limiter import rate_limiter

                fn_map = await rate_limiter.get_workflow_from_number_mapping(
                    int(workflow_run_id)
                )
                if fn_map:
                    fn_org, fn_num, fn_tcid = fn_map
                    if await rate_limiter.release_from_number(
                        fn_org, fn_num, telephony_configuration_id=fn_tcid
                    ):
                ...
                logger.warning(
                    f"[ARI org={self.organization_id}] from_number release failed "
                    f"for run {workflow_run_id}: {exc}"
                )
    ...
    def _map_hangup_cause_to_status(self, cause: int, tech_cause: str = ""):
        """Map an Asterisk Q.850 hangup cause to a telephony call status.

        Used to finalize/dispose an outbound campaign call that never entered
        Stasis (no-answer/busy/failed) — ARI has no status webhook, so the
        ChannelDestroyed event is the only terminal signal for these calls.
        Busy/no-answer are retryable; everything else is a hard failure.
        """
