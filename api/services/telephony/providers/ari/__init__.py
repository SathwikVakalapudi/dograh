...
def _config_loader(value: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "trunk_endpoint": value.get("trunk_endpoint", ""),
...
_UI_METADATA = ProviderUIMetadata(
    ...
    fields=[
        ...
        ProviderUIField(
            name="trunk_endpoint",
            label="Trunk Endpoint",
            type="text",
            required=False,
            description=(
                "Optional. PJSIP endpoint (SIP trunk) name to route outbound calls "
                "through, e.g. 'unpod'. Bare numbers dial as PJSIP/<number>@<endpoint>. "
                "Leave blank if numbers already include an @endpoint suffix."
            ),
        ),
        ProviderUIField(
            name="from_numbers",
