...
class ARIConfigurationRequest(BaseModel):
    ...
    trunk_endpoint: str = Field(
        default="",
        description=(
            "Optional PJSIP endpoint (SIP trunk) to route outbound calls through, "
            "e.g. 'unpod'. Bare numbers are dialed as PJSIP/<number>@<trunk_endpoint>. "
            "Leave blank if your numbers already include an @endpoint suffix."
        ),
    )
