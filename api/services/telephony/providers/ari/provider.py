...
class ARIProvider(TelephonyProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Optional PJSIP endpoint (SIP trunk) that bare outbound numbers are
        # routed through, e.g. "unpod" -> PJSIP/<number>@unpod. Without it, a
        # bare number becomes PJSIP/<number>, which Asterisk reads as an endpoint
        # name and rejects with "Allocation failed" when no such endpoint exists.
        self.trunk_endpoint = (config.get("trunk_endpoint") or "").strip()
    ...
    async def initiate_call(
        ...
        **kwargs: Any,
        ...
        if to_number.startswith("SIP/") or to_number.startswith("PJSIP/"):
            # Caller passed a fully-qualified channel string as-is.
            sip_endpoint = to_number
        elif "@" in to_number:
            # Caller already targeted an endpoint (e.g. +91...@unpod).
            sip_endpoint = f"PJSIP/{to_number}"
        elif self.trunk_endpoint:
            # Route a bare number through the configured SIP trunk endpoint.
            sip_endpoint = f"PJSIP/{to_number}@{self.trunk_endpoint}"
        else:
            # Default to PJSIP technology (requires an endpoint named for the number).
            sip_endpoint = f"PJSIP/{to_number}"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # Record channel -> run mapping so ari_manager's ChannelDestroyed
                # teardown can release the campaign caller-ID/slot for calls that
                # never enter Stasis (no-answer/busy). ARI has no status webhook.
                if channel_id and workflow_run_id is not None:
                    from api.services.campaign.rate_limiter import rate_limiter

                    await rate_limiter.store_channel_run_mapping(
                        channel_id, workflow_run_id
                    )
