...
def create_realtime_llm_service(user_config, audio_config: "AudioConfig"):
    ...
    if provider == ServiceProviders.OPENAI_REALTIME.value:
        ...
        # Pin the recognition language when configured. Any key in `extra` is merged
        # into the Ultravox Create-Call body (llm.py `request_body | params.extra`),
        # so `languageHint` becomes a top-level call field. Without it Ultravox
        # auto-detects language per utterance and mis-transcribes Hindi (as ur/zh/pt).
        ultravox_extra = {"languageHint": language} if language else {}
