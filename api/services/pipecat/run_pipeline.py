...
async def _run_pipeline_impl(
    ...
    user_mute_strategies = [
        # MuteUntilFirstBotCompleteUserMuteStrategy() muted the caller until the
        # first bot utterance (the greeting) finished — making the greeting
        # un-interruptible. Disabled so callers can barge in during the greeting
        # too; whether the greeting is actually interruptible then falls to each
        # node's allow_interrupt (Consent has it on). Restore this line to
        # re-protect the opening if greetings start self-interrupting on noise/echo.
        # MuteUntilFirstBotCompleteUserMuteStrategy(),
        FunctionCallUserMuteStrategy(),
